...
describe('Integration Test: Registration Flow', () => {
    ...
    describe('Complete Registration Flow', () => {
        it('should register � login � verify token � access protected endpoint', async () => {
            ...
            const dbUser = await User_1.UserModel.findByEmail(userData.email);
            expect(dbUser).toBeDefined();
            expect(dbUser?.email).toBe(userData.email);
            expect(dbUser?.phone_number).toBe(userData.phone);
            // Verify password is hashed correctly
            const isValidPassword = await (0, password_1.comparePassword)(userData.password, dbUser.password_hash);
            expect(isValidPassword).toBe(true);
            // Verify verification record was created
            const verification = await Verification_1.VerificationModel.findByUserId(dbUser.id);
            ...
            const tokenResult = (0, jwt_1.verifyToken)(loginTokens.accessToken);
        ...
        it('should prevent duplicate email registration', async () => {
            ...
            // First registration
            const firstResponse = await (0, supertest_1.default)(app_1.app)
                .post('/api/auth/register')
                .send(userData);
            expect(firstResponse.status).toBe(201);
            // Second registration with same email
            const secondResponse = await (0, supertest_1.default)(app_1.app)
                .post('/api/auth/register')
                .send(userData);
    ...
    describe('CRITICAL: Child PII Validation', () => {
        it('should reject registration with prohibited child PII fields', async () => {
            ...
            const dbUser = await User_1.UserModel.findByEmail(invalidData.email);
    ...
    describe('Password Security', () => {
        it('should hash password with bcrypt cost factor 12', async () => {
            ...
            const isValid = await (0, password_1.comparePassword)(userData.password, dbUser.password_hash);
    ...
    describe('JWT Token Management', () => {
        it('should store refresh token in Redis with proper TTL', async () => {
            ...
            const storedToken = await redis_1.default.get(`refresh_token:${user.id}`);
            expect(storedToken).toBe(tokens.refreshToken);
            // Verify TTL is set (should be 7 days = 604800 seconds)
            const ttl = await redis_1.default.ttl(`refresh_token:${user.id}`);
...
//# sourceMappingURL=auth-registration.integration.test.js.map