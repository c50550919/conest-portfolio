...
describe('Complete User Journey - E2E', () => {
    ...
    beforeAll(async () => {
        ...
        // Clear all tables
        await (0, database_1.default)('payments').del();
        await (0, database_1.default)('household_members').del();
        await (0, database_1.default)('households').del();
        await (0, database_1.default)('messages').del();
        await (0, database_1.default)('conversations').del();
        await (0, database_1.default)('matches').del();
        await (0, database_1.default)('verifications').del();
    ...
    describe('Step 1: User Registration', () => {
        it('should register two users successfully', async () => {
            // Register User 1
            const user1Response = await (0, supertest_1.default)(server_1.default)
            ...
            // Register User 2
            const user2Response = await (0, supertest_1.default)(server_1.default)
                .post('/api/auth/register')
                .send({
    ...
    describe('Step 4: Find Matches', () => {
        it('User 1 should find User 2 as potential match', async () => {
            ...
            const matchWithUser2 = response.body.find((m) => m.profile_id === profile2Id);
        ...
        it('User 2 should find User 1 as potential match', async () => {
            const response = await (0, supertest_1.default)(server_1.default)
                ...
                .set('Authorization', `Bearer ${user2Token}`)
                .expect(200);
            const matchWithUser1 = response.body.find((m) => m.profile_id === profile1Id);
    ...
    describe('Complete Journey Summary', () => {
        it('should have completed full lifecycle successfully', async () => {
            // Verify users exist
            const users = await (0, database_1.default)('users').whereIn('id', [user1Id, user2Id]);
            expect(users.length).toBe(2);
            // Verify profiles exist
            const profiles = await (0, database_1.default)('profiles').whereIn('id', [profile1Id, profile2Id]);
            expect(profiles.length).toBe(2);
            // Verify mutual match exists
            const match = await (0, database_1.default)('matches').where({ id: matchId }).first();
            expect(match.status).toBe('mutual_interest');
            // Verify conversation exists
            const conversation = await (0, database_1.default)('conversations')
                .where({ id: conversationId })
                .first();
            expect(conversation).toBeDefined();
            // Verify household exists with 2 members
            const household = await (0, database_1.default)('households').where({ id: householdId }).first();
            expect(household).toBeDefined();
            const members = await (0, database_1.default)('household_members')
